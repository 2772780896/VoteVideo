...
import { createTag } from '../createData/tag'

const searchTagList = createTag(16*3)

export const searchTag = Mock.mock(
    /^\/api\/tag\/search(\?.*)?$/,
    'get',
    function(options) {
        ...
        if (sort === 1) {
            // 实现观看数从高到低排序
            const viewSortList = [...searchTagList].sort((a,b) => {
        ...
        if (sort === 2){
            // 实现时间从新到旧排序
            const dateOrderList = [...searchTagList].sort((a,b) => {
        ...
        if (sort === 3){
            // 实现时间从旧到新排序
            const dateReverseList = [...searchTagList].sort((a,b) => {
                if (a.date > b.date) return 1
                if (b.date > a.date) return -1
                return 0
            })
        ...
        if (sort === 4){
            // 实现点赞数从高到低排序
            const likeSortList = [...searchTagList].sort((a,b) => {
        ...
        if (sort === 5){
            // 实现收藏数从高到低排序
            const favouriteSortList = [...searchTagList].sort((a,b) => {
