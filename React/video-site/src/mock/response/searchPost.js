...
import { createPost } from '../createData/post'

const searchPostList = createPost(16*3)

export const searchPost = Mock.mock(
    /^\/api\/post\/search(\?.*)?$/,
    'get',
    function(options) {
        ...
        if (sort === 1) {
            // 实现观看数从高到低排序
            const viewSortList = [...searchPostList].sort((a,b) => {
        ...
        if (sort === 2){
            // 实现时间从新到旧排序
            const dateOrderList = [...searchPostList].sort((a,b) => {
        ...
        if (sort === 3){
            // 实现时间从旧到新排序
            const dateReverseList = [...searchPostList].sort((a,b) => {
                if (a.date > b.date) return 1
                if (b.date > a.date) return -1
                return 0
            })
        ...
        if (sort === 4){
            // 实现点赞数从高到低排序
            const likeSortList = [...searchPostList].sort((a,b) => {
        ...
        if (sort === 5){
            // 实现收藏数从高到低排序
            const favouriteSortList = [...searchPostList].sort((a,b) => {
