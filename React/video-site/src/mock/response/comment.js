...
export const comment = Mock.mock(
    ...
    function(options) {
        ...
        if (sort === 1) {
            // 实现粉丝数从高到低排序
            const viewSortList = [...commentList].sort((a,b) => {
        ...
        if (sort === 2){
            // 实现时间从新到旧排序
            const dateOrderList = [...commentList].sort((a,b) => {
        ...
        if (sort === 3){
            // 实现时间从旧到新排序
            const dateReverseList = [...commentList].sort((a,b) => {
                if (a.date > b.date) return 1
                if (b.date > a.date) return -1
                return 0
            })
        ...
        if (sort === 4){
            // 实现点赞数从高到低排序
            const likeSortList = [...commentList].sort((a,b) => {
        ...
        if (sort === 5){
            // 实现收藏数从高到低排序
            const favouriteSortList = [...commentList].sort((a,b) => {
