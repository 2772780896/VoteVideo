...
export const addUser = (dataList) => { 
    ...
    userExtension.push(...dataList.map(user => ({
        ...
        message: {
            dialogueList: [],
            notificationList: [],  // @消息 + 收到的赞 + 系统通知 合并
        },
    })))
}
...
export const addVideo = (dataList) => { 
    ...
    for (let i of dataList) {
        ...
        addUserExtension(user.uid, 'uploads', 'videos', i)
    }
}
...
export const addEssay = (dataList) => { 
    ...
    for (let i of dataList) {
        const user = i.uploader
        addUserExtension(user.uid, 'uploads', 'essays', i)
    }
}
...
export const addPost = (dataList) => { 
    ...
    for (let i of dataList) {
        const user = i.uploader
        addUserExtension(user.uid, 'uploads', 'posts', i)
    }
}
